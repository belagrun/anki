...
function suggestNextCharacter(field, current, reference, event) {
  if (
    field.hasClass("st-ok") ||
    field.hasClass("st-error") ||
  ...
  if (
    ...
    (isMacOS() && event.metaKey && event.shiftKey && event.key === "/")
  ) {
...
function countAnsweredFields() {
  ...
  for (let i = 0; i < totalFields; i++) {
    ...
    if (
      field &&
      (field.classList.contains("st-ok") ||
        field.classList.contains("st-wrong-rect"))
    ) {
      answered++;
    }
  }
...
function setUpFillBlankListener(expected, typeAnsIndex) {
  ...
  if (inputEl && inputEl.tagName === "SELECT") {
    inputEl.addEventListener("change", (evt) =>
      checkFieldValue(expected, typeAnsIndex, evt),
    );
  } else {
    inputEl.addEventListener(eventType, (evt) =>
      checkFieldValue(expected, typeAnsIndex, evt),
    );
...
function getRevealPopup() {
  ...
  popup
    .querySelector(".ftb-btn-ok")
    .addEventListener("click", () => applyRevealChoice(true));
  popup
    .querySelector(".ftb-btn-wrong")
    .addEventListener("click", () => applyRevealChoice(false));
